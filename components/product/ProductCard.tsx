...
export default function ProductCard({ id, name, brand, price, originalPrice, image, tags, rating, reviewsCount }: ProductCardProps) {
    return (
            ...
            <div className="relative aspect-square mb-4 overflow-hidden bg-gray-50">
                ...
                {tags && tags.length > 0 && (
                    <div className="absolute top-2 left-2 flex flex-col gap-2">
                        {tags.map((tag) => (
                            <span key={tag} className="bg-white border border-gray-200 text-[10px] font-medium px-2 py-1 uppercase tracking-wider rounded-full">
                                {tag}
                            </span>
                        ))}
                    </div>
