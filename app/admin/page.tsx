...
const DeleteConfirmModal: React.FC<DeleteModalProps> = ({ isOpen, onClose, onConfirm, title, loading }) => {
    return (
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    >
                        ...
                        <h3 className="text-2xl font-serif text-text-body mb-3">Delete {title}?</h3>
...
const ProductManager: React.FC = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        console.log('Product Save Started', formData);
        setLoading(true);
        try {
            const payload = {
                ...formData,
            ...
            setLoading(false);
            console.log('Product Save Finished');
        }
    };
...
const GalleryManager: React.FC = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        console.log('Gallery Save Started', formData);
        setLoading(true);
        try {
            const payload = {
                ...formData,
            ...
            setLoading(false);
            console.log('Gallery Save Finished');
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-text-muted text-sm">Manage the gallery items displayed on the home page.</p>
                </div>
                <Button
                    onClick={() => {
                        setFormData({ title: '', image: '', link: '/shop', description: '' });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {galleryItems.map((item) => (
                    ...
                    >
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                            ...
                            <p className="text-xs opacity-70 truncate mb-4">{item.link || 'No link set'}</p>
                ...
                {galleryItems.length === 0 && (
                    <div className="col-span-full py-12 text-center bg-white rounded-xl border border-dashed border-gray-300">
                        <p className="text-gray-500">No collection items found. Add one to get started.</p>
                    </div>
                )}
            </div>
            ...
            <aside className="w-full md:w-64 bg-white border-r border-gray-200 flex flex-col h-auto md:h-screen sticky top-0 z-20">
                ...
                <nav className="flex-1 p-4 space-y-1">
                    ...
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeTab === 'gallery'
                            ? 'bg-terracotta text-white shadow-md'
                            : 'text-gray-600 hover:bg-gray-50'
                            }`}
                    >
