...
import { Minus, Plus, Trash2, ArrowLeft, PenTool, MapPin, Check } from 'lucide-react';
    ...
    React.useEffect(() => {
        if (userProfile?.addresses && userProfile.addresses.length > 0) {
            const lastUsed = userProfile.lastUsedAddressId
                ? userProfile.addresses.find((a) => a.id === userProfile.lastUsedAddressId)
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            if (user && userProfile) {
                const userRef = doc(db, 'users', user.uid);
                const updates: any = {
                    lastUsedAddressId: selectedAddressId === 'new' ? '' : selectedAddressId,
                };
                ...
                if (selectedAddressId === 'new' && saveToAccount) {
                    ...
                    updates.addresses = [...(userProfile.addresses || []), newAddressWithId];
                                    ...
                                    <div className="mb-8 border-b border-gray-100 pb-8">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div
                                                ...
                                                className={`p-4 rounded-xl border-2 border-dashed flex items-center justify-center gap-2 cursor-pointer transition-all ${selectedAddressId === 'new'
                                                    ? 'border-terracotta bg-cream/20'
                                                    : 'border-gray-200 hover:border-gray-300'
                                                    }`}
                                            >
                                                <Plus size={18} className={selectedAddressId === 'new' ? 'text-terracotta' : 'text-gray-400'} />
                                                <span className={`text-sm font-medium ${selectedAddressId === 'new' ? 'text-terracotta' : 'text-gray-500'}`}>
                                                    Add New Address
                                                </span>
                                            </div>
                                        </div>
