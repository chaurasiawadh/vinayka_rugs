...
const Watchlist: React.FC = () => {
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-cream px-4 text-center">
        ...
        <p className="text-text-muted mb-8 max-w-md">
          Login to view your saved items, track prices, and get back-in-stock
          alerts.
        </p>
  ...
  const watchedProducts = wishlist
    .map((item) => {
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-cream px-4 text-center">
        ...
        <p className="text-text-muted mb-6">
          Save items you love to get notified about price drops.
        </p>
        <Link href="/shop">
    ...
    <div className="bg-cream min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-serif mb-8">
          My Watchlist ({watchedProducts.length})
        </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {watchedProducts.map((item: any) => (
              <div
                key={item.id}
                className="bg-white p-6 rounded-xl shadow-sm flex flex-col sm:flex-row gap-6 animate-fade-in group"
              >
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <Link
                        ...
                        className="font-serif text-xl hover:text-terracotta transition-colors"
                      >
                        {item.name}
                      </Link>
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 cursor-pointer group/opt">
                        <input
                          ...
                          onChange={(e) =>
                            updateWishlistPreferences(item.id, {
                              priceDrop: e.target.checked,
                            })
                          }
                          className="accent-terracotta"
                        />
                        <span className="text-xs text-text-muted group-hover/opt:text-terracotta transition-colors">
                          Alert: Price Drop
                        </span>
                      </label>
                    </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-sm sticky top-24">
              ...
              <p className="text-sm text-text-muted mb-6">
                You have {watchedProducts.length} items saved. Enable
              ...
              <div className="bg-teal/5 border border-teal/10 rounded-lg p-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-teal-700 mt-1">
                    We will email you at {user.email}
                  </p>
                </div>
              </div>
