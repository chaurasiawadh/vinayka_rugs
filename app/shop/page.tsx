    ...
    return products
      .filter((product) => {
        ...
        const matchMat =
          ...
          (productMaterial && selectedMaterials.includes(productMaterial));
        const matchCol =
          !initialCollection || productCollection === initialCollection;
              ...
              <div>
                ...
                <div className="space-y-2">
                  {CATEGORIES.map((cat) => (
                    ...
                    >
                      <div
                        className={`w-4 h-4 border rounded flex items-center justify-center transition-colors ${selectedCategories.includes(cat) ? 'bg-terracotta border-terracotta' : 'border-gray-300 group-hover:border-terracotta'}`}
                      >
                      ...
                      />
                      <span
                        className={`text-sm ${selectedCategories.includes(cat) ? 'text-text-body font-medium' : 'text-text-muted'}`}
                      >
                        {cat}
                      </span>
                    </label>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {MATERIALS.map((mat) => (
                    ...
                    >
                      <div
                        className={`w-4 h-4 border rounded flex items-center justify-center transition-colors ${selectedMaterials.includes(mat) ? 'bg-terracotta border-terracotta' : 'border-gray-300 group-hover:border-terracotta'}`}
                      >
                      ...
                      />
                      <span
                        className={`text-sm ${selectedMaterials.includes(mat) ? 'text-text-body font-medium' : 'text-text-muted'}`}
                      >
                        {mat}
                      </span>
                    </label>
