...
import { MapPin, Phone, Clock, Mail, FileText, MessageCircle } from 'lucide-react';
...
const LOCATIONS: StoreLocation[] = [
  ...
  },
  {
    ...
    city: 'Bhadohi',
    name: 'SHYAMA EXPORTS',
    address: ['Chauri Bazar (Main Road), Parsipur', 'Bhadohi-221 402, (U.P.) INDIA'],
    hours: 'Mon - Sat : 9am - 7pm',
    phone: '+91-9125007797, +91-9580122122',
                ...
                <div className="space-y-6 text-text-muted mb-8 font-light leading-relaxed">
                  ...
                  <div className={`flex items-center gap-3 ${index % 2 !== 0 ? 'flex-row-reverse md:flex-row-reverse' : ''}`}>
                    ...
                    <div className={`flex flex-col ${index % 2 !== 0 ? 'items-end' : 'items-start'}`}>
                      {location.phone.split(',').map((p, i) => (
                        <a key={i} href={`tel:${p.trim()}`} className="hover:text-terracotta transition-colors underline decoration-1 underline-offset-4 block">
                          {p.trim()}
                        </a>
                      ))}
                    </div>
                  ...
                  {location.whatsapp && (
                    <div className={`flex items-center gap-3 ${index % 2 !== 0 ? 'flex-row-reverse md:flex-row-reverse' : ''}`}>
                      ...
                      <a href={`https://wa.me/${location.whatsapp}`} target="_blank" rel="noopener noreferrer" className="hover:text-terracotta transition-colors underline decoration-1 underline-offset-4">
                        {location.whatsapp}
                      </a>
                    </div>
                  )}
                  ...
                  {location.email && (
                    <div className={`flex items-center gap-3 ${index % 2 !== 0 ? 'flex-row-reverse md:flex-row-reverse' : ''}`}>
                      ...
                      <a href={`mailto:${location.email}`} className="hover:text-terracotta transition-colors underline decoration-1 underline-offset-4 break-all">
                        {location.email}
                      </a>
                    </div>
                  )}
                  ...
                  {location.gstin && (
                    <div className={`flex items-center gap-3 ${index % 2 !== 0 ? 'flex-row-reverse md:flex-row-reverse' : ''}`}>
                      ...
                      <span className="text-text-muted text-sm">GSTIN: {location.gstin}</span>
                    </div>
                  )}
                </div>
