...
import { User, Mail, Phone, MapPin, Briefcase, Building, Calendar, LogOut, Loader, ShoppingBag, Edit2, Save, X } from 'lucide-react';
...
const AccountPage = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (formData.firstName !== userProfile.firstName || formData.lastName !== userProfile.lastName) {
        ...
        <div className="bg-gray-50 min-h-screen pb-20">
            ...
            <div className="bg-gray-900 text-white pt-24 pb-20 px-6">
                <div className="max-w-6xl mx-auto">
                    ...
                    <p className="text-gray-400 font-light">Manage your profile and view your orders</p>
                </div>
            </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
                            <div className="p-8 text-center border-b border-gray-100">
                                ...
                                <h2 className="text-xl font-bold font-serif text-gray-900">{userProfile.firstName} {userProfile.lastName}</h2>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8 animate-fade-in">
                            <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-50">
                                <h3 className="text-lg font-bold font-serif flex items-center gap-2 text-gray-800">
                                    <User size={20} className="text-terracotta" /> Profile Information
                                </h3>
                                {!isEditing ? (
                                    <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 text-sm font-medium text-terracotta hover:underline">
                                        <Edit2 size={16} /> Edit
                                    </button>
                                ) : (
                                    <div className="flex gap-2">
                                        <button onClick={() => { setIsEditing(false); setFormData(userProfile); }} className="p-2 text-gray-400 hover:bg-gray-100 rounded-full"><X size={18} /></button>
                                        <button onClick={handleSave} disabled={saving} className="flex items-center gap-1.5 px-3 py-1.5 bg-terracotta text-white rounded text-sm hover:bg-black transition-colors">
                                            {saving ? <Loader size={12} className="animate-spin" /> : <Save size={14} />} Save
                                        </button>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                                ...
                                {isEditing ? (
                                    ...
                                    <DetailItem icon={User} label="First Name" value={userProfile.firstName} />
                                )}
                                ...
                                {isEditing ? (
                                    ...
                                    <DetailItem icon={User} label="Last Name" value={userProfile.lastName} />
                                )}

                                <DetailItem icon={Mail} label="Email Address" value={userProfile.email} />
                                ...
                                {isEditing ? (
                                    ...
                                    <DetailItem icon={Phone} label="Phone Number" value={userProfile.phone || 'Not Provided'} />
                                )}

                                {isEditing ? (
                                    <div className="col-span-1">
                                        <label className="text-[10px] font-bold uppercase text-gray-500 mb-1 block">Profession</label>
                                        <input
                                            name="profession"
                                    ...
                                    <DetailItem icon={Briefcase} label="Profession" value={userProfile.profession || 'Not Specified'} />
                                )}

                                {isEditing ? (
                                    <div className="col-span-1">
                                        <label className="text-[10px] font-bold uppercase text-gray-500 mb-1 block">Company</label>
                                        <input
                                            name="companyName"
                                    ...
                                    <DetailItem icon={Building} label="Company" value={userProfile.companyName || '-'} />
                                )}

                                {isEditing ? (
                                    <div className="col-span-1">
                                        <label className="text-[10px] font-bold uppercase text-gray-500 mb-1 block">City</label>
                                        <input
                                            name="city"
                                            value={formData.city || ''}
                                            onChange={handleInputChange}
                                    ...
                                    <DetailItem icon={MapPin} label="City" value={userProfile.city || '-'} />
                                )}

                                <DetailItem icon={Calendar} label="Member Since" value={memberSince} />
                            </div>
                        ...
                        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8 animate-fade-in animation-delay-100">
                            <h3 className="text-lg font-bold font-serif mb-6 flex items-center gap-2 text-gray-800">
                                <ShoppingBag size={20} className="text-terracotta" /> Recent Orders
                            </h3>
                            ...
                            <div className="text-center py-12 px-4 bg-gray-50/50 rounded-xl border-2 border-dashed border-gray-200">
                                ...
                                <p className="text-gray-500 text-sm mb-6 max-w-xs mx-auto">Looks like you haven't placed an order for our exquisite rugs yet.</p>
                                <Button size="sm" onClick={() => router.push('/shop')}>Explore Collection</Button>
                            </div>
                        </section>
                    </div>
                </div>
...
const DetailItem = ({ icon: Icon, label, value }: { icon: any, label: string, value: string }) => (
    <div className="flex items-start gap-4 group">
        ...
        <div>
            <p className="text-[10px] font-bold uppercase text-gray-400 mb-1 tracking-wider">{label}</p>
