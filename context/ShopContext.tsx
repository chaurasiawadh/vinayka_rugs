...
const mergeCarts = (local: CartItem[], remote: CartItem[]) => {
  ...
  local.forEach((lItem) => {
    const existingIndex = merged.findIndex(
      (rItem) =>
        rItem.id === lItem.id && rItem.selectedSize === lItem.selectedSize
    );
    if (existingIndex > -1) {
...
interface ShopContextType {
  ...
  updateWishlistPreferences: (
    productId: string,
    prefs: Partial<WatchlistItem['preferences']>
  ) => void;
...
export const ShopProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    const syncUserCart = async () => {
      ...
      const localItems = JSON.parse(
        localStorage.getItem('vinayka_cart') || '[]'
      );
  ...
  const updateWishlistPreferences = async (
    productId: string,
    prefs: Partial<WatchlistItem['preferences']>
  ) => {
    if (!user) return;
