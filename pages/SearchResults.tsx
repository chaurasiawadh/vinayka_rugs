    ...
    <div className="bg-cream min-h-screen pb-20 pt-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 border-b border-gray-200 pb-6">
          <div>
            <span className="text-sm text-text-muted uppercase tracking-wider">Search Results</span>
            <h1 className="text-3xl font-serif text-text-body mt-2">
              {q ? `Results for "${q}"` : 'All Products'}
            </h1>
            <p className="text-text-muted mt-2">{results.totalCount} items found</p>
          </div>
        ...
        <div className="flex gap-8">
          ...
          <aside className={`w-64 flex-shrink-0 ${isMobileFilterOpen ? 'fixed inset-0 z-50 bg-white p-6 overflow-y-auto' : 'hidden md:block'}`}>
            ...
            {Object.keys(selectedFilters).some(k => selectedFilters[k].length > 0) && (
                <div className="mb-6">
                    <button onClick={clearFilters} className="text-sm text-terracotta hover:underline">Clear All</button>
                </div>
            )}
